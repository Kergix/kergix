...
export default async function IndustryDetailPage({ params }: IndustryDetailPageProps) {
  ...
  const renderIcon = (iconName: string) => {
    return <IconRenderer name={iconName} className="w-8 h-8 text-white" />;
  };
      ...
      <section className="relative pt-36 pb-20 px-4 md:px-8 bg-bg-secondary border-b border-border-subtle">
        ...
        <div className="max-w-7xl mx-auto flex flex-col gap-6 relative z-10">
          ...
          <div className="flex items-center gap-4">
            <div className="p-3.5 bg-white/5 border border-white/20 rounded-xl backdrop-blur-sm">
            ...
            {industry.solutions.map((sol, idx) => (
              <div
                ...
                className={`p-6 md:p-8 rounded-2xl flex flex-col gap-5 justify-between transition-all duration-300 ${
                  sol.highlighted
                    ? "border border-white/30 bg-white/10 shadow-[0_0_25px_rgba(255,255,255,0.12)] md:scale-105"
                    : "border border-white/10 bg-white/5 hover:border-white/30"
                }`}
              >
                <div className="flex flex-col gap-4">
                  <span className={`font-heading font-bold text-sm text-white/70 uppercase tracking-wider`}>
                    Solution 0{idx + 1}
                  </span>

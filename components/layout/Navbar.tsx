      ...
      >
        <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between relative">
          ...
          <Link href="/" className="flex items-center gap-2.5 group/logo" aria-label="Kergix home page">
            ...
            />
            <span className="font-heading font-bold text-xl text-text-primary tracking-tight transition-colors group-hover/logo:text-accent-cyan-bright">
              Kergix
            </span>
          </Link>
                ...
                >
                  <Link
                    ...
                    className={`relative px-4 py-2 text-sm font-semibold flex items-center gap-1.5 transition-colors focus:outline-none ${isItemActive
                        ? "text-text-primary"
                        : "text-text-secondary hover:text-accent-cyan-bright"
                      }`}
                  >
                    ...
                    {isItemActive && (
                      <span className="absolute -bottom-0.5 left-4 right-4 h-0.5 rounded-full bg-accent-cyan shadow-[0_0_8px_rgba(0,230,250,0.6)]" />
                    )}
                  </Link>
                          ...
                          {item.children?.map((child) => (
                            <Link
                              ...
                              className={`p-3 rounded-lg flex flex-col gap-1 transition-all ${pathname === child.href
                                  ? "bg-accent-cyan/8 border border-accent-cyan/25"
                                  : "hover:bg-accent-cyan/5 hover:border-accent-cyan/15 border border-transparent"
                                }`}
                            >
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div className="absolute right-[-10%] top-[-10%] w-[300px] h-[300px] bg-accent-cyan/6 rounded-full blur-[80px]" />
            <div className="absolute left-[-10%] bottom-[-10%] w-[300px] h-[300px] bg-accent-teal-deep/8 rounded-full blur-[80px]" />
            ...
            <div className="flex flex-col gap-6 w-full max-w-lg mx-auto pb-10 z-10">
              ...
              <Link
                href="/contact"
                onClick={() => setIsOpen(false)}
                className="mt-6 w-full py-4 rounded-xl bg-gradient-to-r from-accent-cyan-bright to-accent-cyan-mid text-[#03181E] font-heading font-bold text-base text-center flex items-center justify-center gap-2 shadow-lg shadow-accent-cyan/25"
              >
